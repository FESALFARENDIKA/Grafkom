...
class KurvaKonikApp:
    ...
    def draw_info_box_and_legend(self):
        ...
        # Legend
        x0, y0 = 10, h - 70
        self.canvas.create_rectangle(x0, y0, x0+120, y0+60, fill=self.panel_bg, outline=self.grid_color)
        self.canvas.create_text(x0+8, y0+8, text="Legenda", fill=self.bg_dark, font=("Arial", 9, "bold"), anchor="nw")
        self.canvas.create_oval(x0+10, y0+25, x0+20, y0+35, fill=self.point_color, outline="")
        self.canvas.create_text(x0+28, y0+23, text="Titik Biasa", fill=self.bg_dark, font=("Arial", 8), anchor="nw")
        self.canvas.create_oval(x0+10, y0+42, x0+20, y0+52, fill=self.active_color, outline="")
        self.canvas.create_text(x0+28, y0+40, text="Titik Aktif", fill=self.bg_dark, font=("Arial", 8), anchor="nw")
        
        # Info Box Parameter
        x1, y1 = w - 170, h - 90
        self.canvas.create_rectangle(x1, y1, x1+160, y1+80, fill=self.panel_bg, outline=self.grid_color)
        self.canvas.create_text(x1+8, y1+8, text="Info Canvas", fill=self.bg_dark, font=("Arial", 9, "bold"), anchor="nw")
        info = f"Zoom: {self.scale:.1f}x\n"
        info += f"Kurva: {self.current_curve.get()}\n"
        if self.frames:
            info += f"Total Titik: {len(self.frames)}\n"
            info += f"Ditampilkan: {self.visible_count}"
        self.canvas.create_text(x1+8, y1+25, text=info, fill=self.bg_dark, font=("Consolas", 8), anchor="nw")
