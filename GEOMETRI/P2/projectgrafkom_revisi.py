...
class KurvaKonikApp:
    ...
    def bind_keyboard(self):
        self.root.bind_all("<space>", lambda event: self.toggle_pause())
        self.root.bind_all("<Right>", lambda event: self.next_interval())
        self.root.bind_all("<Left>", lambda event: self.previous_interval())
        self.root.bind_all("<Home>", lambda event: self.go_home())
        self.root.bind_all("<End>", lambda event: self.finish_curve())
        self.root.bind_all("<r>", lambda event: self.reset_all())
        self.root.bind_all("<R>", lambda event: self.reset_all())
        self.root.bind_all("<Up>", lambda event: self.increase_step())
        self.root.bind_all("<Down>", lambda event: self.decrease_step())

        self.root.bind_all("1", lambda event: self.change_curve("Lingkaran"))
        self.root.bind_all("2", lambda event: self.change_curve("Elips"))
        self.root.bind_all("3", lambda event: self.change_curve("Parabola"))
        self.root.bind_all("4", lambda event: self.change_curve("Hiperbola"))

        self.root.bind_all("+", lambda event: self.zoom_in())
        self.root.bind_all("=", lambda event: self.zoom_in())
        self.root.bind_all("-", lambda event: self.zoom_out())

        self.root.bind_all("<w>", lambda event: self.pan(0, 25))
        self.root.bind_all("<W>", lambda event: self.pan(0, 25))
        self.root.bind_all("<s>", lambda event: self.pan(0, -25))
        self.root.bind_all("<S>", lambda event: self.pan(0, -25))
        self.root.bind_all("<a>", lambda event: self.pan(25, 0))
        self.root.bind_all("<A>", lambda event: self.pan(25, 0))
        self.root.bind_all("<d>", lambda event: self.pan(-25, 0))
        self.root.bind_all("<D>", lambda event: self.pan(-25, 0))
